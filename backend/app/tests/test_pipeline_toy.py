...
def _upload_all_toy_inputs(role: str = "admin") -> None:
    with open("example/customers_toy.csv", "rb") as f:
        resp = client.post("/upload/customers", headers={"X-User-Role": role}, files={"file": ("customers_toy.csv", f, "text/csv")})
    assert resp.status_code == 200

    with open("example/sites_toy.csv", "rb") as f:
        resp = client.post("/upload/sites", headers={"X-User-Role": role}, files={"file": ("sites_toy.csv", f, "text/csv")})
    assert resp.status_code == 200

    with open("example/tax_rules_toy.json", "rb") as f:
        resp = client.post("/upload/tax_rules", headers={"X-User-Role": role}, files={"file": ("tax_rules_toy.json", f, "application/json")})
    assert resp.status_code == 200
...
def test_upload_endpoints_rbac_forbidden() -> None:
    with open("example/customers_toy.csv", "rb") as f:
        resp = client.post("/upload/customers", headers={"X-User-Role": "viewer"}, files={"file": ("customers_toy.csv", f, "text/csv")})
...
def test_run_analysis_sync_with_consent() -> None:
    ...
    resp = client.post(
        ...
        json={
            "project_id": "proj_toy",
            "params": {"horizon_months": 6, "max_sites": 3, "objective_weights": {"cost": 0.7, "time": 0.3}},
...
def test_run_analysis_rejected_without_consent_when_sensitive_data_present() -> None:
    ...
    # inject sensitive column in uploaded raw customers file for the project
    from app.api.routes import PROJECTS

    cpath = PROJECTS["proj_toy"]["customers.csv"]
    ...
    resp = client.post(
        ...
        json={
            "project_id": "proj_toy",
            "params": {"horizon_months": 6, "max_sites": 3, "objective_weights": {"cost": 0.7, "time": 0.3}},
            "consent_to_use_sensitive_data": False,
        },
    )
